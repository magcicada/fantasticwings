...
public final class FlightDefault implements Flight {
    ...
    public float getFlyingAmount(float delta) {
        return FLY_AMOUNT_CURVE.eval(MathHelper.lerp(this.getPrevTimeFlying(), this.getTimeFlying(), delta) / MAX_TIME_FLYING);
    }
    ...
    private void onWornUpdate(Player player) {
        if (player.isEffectiveAi()) {
            if (this.isFlying()) {
                ...
                float elevationBoost = MathHelper.transform(
                    Math.abs(player.xRot),
                    45.0F, 90.0F,
                    1.0F, 0.0F
                );
                float pitch = -MathHelper.toRadians(player.xRot - PITCH_OFFSET * elevationBoost);
                float yaw = -MathHelper.toRadians(player.yRot) - MathHelper.PI;
