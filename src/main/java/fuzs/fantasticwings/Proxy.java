...
import fuzs.fantasticwings.server.dreamcatcher.InSomniable;
...
import fuzs.fantasticwings.server.net.Network;
import fuzs.fantasticwings.server.net.clientbound.MessageSyncFlight;
import fuzs.fantasticwings.server.potion.PotionMix;
...
public abstract class Proxy {
    ...
    protected void setup(FMLCommonSetupEvent event) {
        ...
        event.enqueueWork(() -> {
            BiConsumer<ItemLike, Holder.Reference<Item>> reg = (item, obj) -> {
                BrewingRecipeRegistry.addRecipe(
                    new PotionMix(Potions.SLOW_FALLING, Ingredient.of(item), new ItemStack(obj.value()))
                );
                BrewingRecipeRegistry.addRecipe(
                    new PotionMix(Potions.LONG_SLOW_FALLING, Ingredient.of(item), new ItemStack(obj.value()))
                );
            };
            reg.accept(Items.FEATHER, WingsItems.ANGEL_WINGS_BOTTLE);
            reg.accept(Items.RED_DYE, WingsItems.PARROT_WINGS_BOTTLE);
            reg.accept(WingsItems.BAT_BLOOD_BOTTLE.value(), WingsItems.BAT_WINGS_BOTTLE);
