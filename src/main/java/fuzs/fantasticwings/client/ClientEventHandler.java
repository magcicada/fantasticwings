...
import fuzs.fantasticwings.server.asm.AnimatePlayerModelEvent;
import fuzs.fantasticwings.server.asm.ApplyPlayerRotationsEvent;
import fuzs.fantasticwings.server.asm.EmptyOffHandPresentEvent;
import fuzs.fantasticwings.server.asm.GetCameraEyeHeightEvent;
...
public final class ClientEventHandler {
    ...
    public static void onAnimatePlayerModel(AnimatePlayerModelEvent event) {
        ...
        Flights.get(player).ifPresent(flight -> {
            ...
            model.head.xRot = MathHelper.toRadians(MathHelper.lerp(pitch, pitch / 4.0F - 90.0F, amt));
            model.leftArm.xRot = MathHelper.lerp(model.leftArm.xRot, -3.2F, amt);
            model.rightArm.xRot = MathHelper.lerp(model.rightArm.xRot, -3.2F, amt);
            model.leftLeg.xRot = MathHelper.lerp(model.leftLeg.xRot, 0.0F, amt);
            model.rightLeg.xRot = MathHelper.lerp(model.rightLeg.xRot, 0.0F, amt);
    ...
    public static void onApplyRotations(ApplyPlayerRotationsEvent event) {
        Flights.ifPlayer(event.getEntity(), (player, flight) -> {
            ...
            if (amt > 0.0F) {
                ...
                matrixStack.mulPose(Vector3f.ZP.rotationDegrees(MathHelper.lerpDegrees(0.0F, roll, amt)));
                matrixStack.mulPose(Vector3f.XP.rotationDegrees(MathHelper.lerpDegrees(0.0F, pitch, amt)));
                matrixStack.translate(0.0D, -1.2D * MathHelper.easeInOut(amt), 0.0D);
            }
        });
    }
    ...
    public static void onCameraSetup(EntityViewRenderEvent.CameraSetup event) {
        Flights.ifPlayer(Minecraft.getInstance().cameraEntity, (player, flight) -> {
            ...
            if (amt > 0.0F) {
                ...
                event.setRoll(MathHelper.lerpDegrees(0.0F, -roll * 0.25F, amt));
            }
        });
    }
